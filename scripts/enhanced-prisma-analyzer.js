...
class ComprehensivePrismaAnalyzer {
  constructor(serverPath = "../server", clientPath = "../client") {
    ...
    this.prismaSchemaPath = path.join(this.serverPath, "prisma", "schema.prisma");
  ...
  parsePrismaSchema() {
    try {
      ...
      for (const [name, model] of this.models) {
        console.log(`  • ${name} (${model.fields.length} fields, ${model.relationships.length} relations)`);
      }
      console.log("");
    } catch (error) {
  ...
  getAllFiles(dirPath, fileExtensions = [".js", ".ts", ".jsx", ".tsx", ".json", ".sql"]) {
    const files = [];
    ...
    for (const item of items) {
      ...
      if (stat.isDirectory()) {
        if (![
          ...
          "public", ".expo", "android", "ios", "web-build", ".vscode"
        ].includes(item)) {
  ...
  analyzeModelUsage(modelName, filePath) {
    ...
    const usageAnalysis = {
      file: isClientFile ? path.relative(this.clientPath, filePath) : relativePath,
      isClientFile,
      usageTypes: new Set(),
      usageCount: 0,
      confidence: 0,
    ...
    if (!isClientFile) {
      const prismaPatterns = [
        ...
        new RegExp(`prisma\\.${modelName.toLowerCase()}\\.(create|createMany|findFirst|findUnique|findMany|update|updateMany|upsert|delete|deleteMany|count|aggregate|groupBy)\\s*\\(`, "gi"),
        new RegExp(`prisma\\.${modelName}\\.(create|createMany|findFirst|findUnique|findMany|update|updateMany|upsert|delete|deleteMany|count|aggregate|groupBy)\\s*\\(`, "g"),
        
        // Transaction operations
        new RegExp(`tx\\.${modelName.toLowerCase()}\\.(create|findFirst|findUnique|findMany|update|updateMany|upsert|delete|deleteMany)`, "gi"),
        new RegExp(`tx\\.${modelName}\\.(create|findFirst|findUnique|findMany|update|updateMany|upsert|delete|deleteMany)`, "g"),
        ...
        // Include statements
        new RegExp(`include:\\s*\\{[^}]*${modelName.toLowerCase()}[^}]*\\}`, "gi"),
    ...
    const apiPatterns = [
      // Express routes
      new RegExp(`\\.(get|post|put|delete|patch)\\s*\\([^)]*['"\`][^'"\`]*${modelName.toLowerCase()}[^'"\`]*['"\`]`, "gi"),
      new RegExp(`\\.(get|post|put|delete|patch)\\s*\\([^)]*['"\`][^'"\`]*${modelName}[^'"\`]*['"\`]`, "g"),
      
      // Route parameters and body
      new RegExp(`req\\.(params|body|query)\\.\\w*${modelName.toLowerCase()}`, "gi"),
      new RegExp(`req\\.(params|body|query)\\.\\w*${modelName}`, "g"),
      
      // API endpoint strings
      new RegExp(`['"\`]/api/[^'"\`]*${modelName.toLowerCase()}[^'"\`]*['"\`]`, "gi"),
      new RegExp(`['"\`]/[^'"\`]*${modelName.toLowerCase()}[^'"\`]*['"\`]`, "gi"),
      
      // Client API calls
      new RegExp(`(axios|fetch)\\s*\\([^)]*['"\`][^'"\`]*${modelName.toLowerCase()}[^'"\`]*['"\`]`, "gi"),
      new RegExp(`(get|post|put|delete|patch)\\s*\\([^)]*['"\`][^'"\`]*${modelName.toLowerCase()}[^'"\`]*['"\`]`, "gi"),
    ];
    ...
    const businessLogicPatterns = [
      ...
      new RegExp(`(function|const|let)\\s+\\w*${modelName.toLowerCase()}\\w*`, "gi"),
      new RegExp(`(function|const|let)\\s+\\w*${modelName}\\w*`, "g"),
      
      // Method calls
      new RegExp(`\\.(get|create|update|delete|fetch|save|load)${modelName}`, "gi"),
      new RegExp(`\\.(get|create|update|delete|fetch|save|load)${modelName.toLowerCase()}`, "gi"),
  ...
  analyzeAllModels() {
    ...
    console.log(`📁 Analyzing ${allFiles.length} files across server and client...`);
    console.log(`   🖥️  Server files: ${allFiles.filter(f => f.startsWith(this.serverPath)).length}`);
    console.log(`   💻 Client files: ${allFiles.filter(f => f.startsWith(this.clientPath)).length}`);
    console.log("");
    ...
    for (const [modelName, modelInfo] of this.models) {
      ...
      for (const filePath of allFiles) {
        ...
        if (analysis) {
          ...
          analysis.usageTypes.forEach(type => modelUsage.usageTypes.add(type));
          modelUsage.fileAnalyses.push(analysis);
        }
      }

      // Determine if model is really used
      const hasStrongUsage = modelUsage.usageTypes.has("DATABASE_OPERATION") || 
                           modelUsage.usageTypes.has("API_ENDPOINT");
      const hasMediumUsage = modelUsage.usageTypes.has("TYPE_DEFINITION") || 
                           modelUsage.usageTypes.has("BUSINESS_LOGIC") ||
                           modelUsage.usageTypes.has("CLIENT_OPERATION");
      const hasWeakUsage = modelUsage.usageTypes.has("WEAK_INDICATOR") || 
      ...
      } else if (hasMediumUsage || (hasWeakUsage && modelUsage.totalConfidence >= 20)) {
      ...
      console.log(`   📊 Confidence: ${modelUsage.totalConfidence}, Files: ${modelUsage.totalFiles}`);
      console.log(`   🖥️  Server: ${modelUsage.serverFiles}, 💻 Client: ${modelUsage.clientFiles}`);
      console.log(`   🏷️  Usage types: ${Array.from(modelUsage.usageTypes).join(", ")}`);
      ...
      if (modelUsage.fileAnalyses.length > 0) {
        ...
        topFiles.forEach(analysis => {
          ...
          console.log(`   ${location} ${analysis.file} (confidence: ${analysis.confidence})`);
        });
      }
      console.log("");
    }
  ...
  generateComprehensiveReport() {
    ...
    const safeModels = modelsByRisk.SAFE.length + modelsByRisk.PROBABLY_SAFE.length;
    const riskyModels = modelsByRisk.SUSPICIOUS.length + modelsByRisk.LIKELY_UNUSED.length;
    const unusedModels = modelsByRisk.DEFINITELY_UNUSED.length;

    console.log(`\n📈 SUMMARY:`);
    console.log(`  Total models: ${totalModels}`);
    console.log(`  ✅ Safe models: ${safeModels} (${((safeModels/totalModels)*100).toFixed(1)}%)`);
    console.log(`  🟡 Risky models: ${riskyModels} (${((riskyModels/totalModels)*100).toFixed(1)}%)`);
    console.log(`  ❌ Unused models: ${unusedModels} (${((unusedModels/totalModels)*100).toFixed(1)}%)`);
    ...
    Object.entries(modelsByRisk).forEach(([riskLevel, models]) => {
      if (models.length > 0) {
        ...
        models.forEach(({ name, usage }) => {
          console.log(`   • ${name}`);
          console.log(`     📊 Confidence: ${usage.totalConfidence}, Files: ${usage.totalFiles}`);
          console.log(`     🏷️  Types: ${Array.from(usage.usageTypes).join(", ")}`);
          
          if (usage.fileAnalyses.length > 0) {
            const topFile = usage.fileAnalyses.sort((a, b) => b.confidence - a.confidence)[0];
    ...
    if (riskyModels > 0) {
      ...
      console.log(`      • Review ${riskyModels} suspicious/likely unused model(s)`);
      [...modelsByRisk.SUSPICIOUS, ...modelsByRisk.LIKELY_UNUSED].forEach(({ name, usage }) => {
        console.log(`        - ${name} (confidence: ${usage.totalConfidence})`);
      });
    }
  ...
  analyzeRelationships(modelsByRisk) {
    const unusedModels = new Set(modelsByRisk.DEFINITELY_UNUSED.map(m => m.name));
    const riskyModels = new Set([
      ...modelsByRisk.SUSPICIOUS.map(m => m.name),
      ...modelsByRisk.LIKELY_UNUSED.map(m => m.name),
    ]);

    console.log(`   🔍 Checking relationships for ${unusedModels.size + riskyModels.size} at-risk models...`);
      ...
      if (unusedModels.has(modelName) || riskyModels.has(modelName)) {
        ...
        for (const [otherModelName, otherModelInfo] of this.models) {
          if (otherModelName !== modelName) {
            const hasRelation = otherModelInfo.relationships.some(rel => rel.model === modelName);
            if (hasRelation) {
        ...
        if (relatedModels.length > 0) {
          ...
          console.log(`   ⚠️  ${riskLevel} model ${modelName} is referenced by: ${relatedModels.join(", ")}`);
          
          // Check if related models are also unused
          const safeRelations = relatedModels.filter(rel => 
            !unusedModels.has(rel) && !riskyModels.has(rel)
          );
          
          if (safeRelations.length > 0) {
            console.log(`      🚨 DANGER: Referenced by actively used models: ${safeRelations.join(", ")}`);
          }
        }
      }
    }
  ...
  exportResults(outputPath = "../comprehensive-prisma-analysis.json") {
    const report = {
      ...
      summary: {
        ...
        safeModels: Array.from(this.modelUsage.values()).filter(u => 
          u.riskLevel === "SAFE" || u.riskLevel === "PROBABLY_SAFE"
        ).length,
        riskyModels: Array.from(this.modelUsage.values()).filter(u => 
          u.riskLevel === "SUSPICIOUS" || u.riskLevel === "LIKELY_UNUSED"
        ).length,
        unusedModels: Array.from(this.modelUsage.values()).filter(u => 
          u.riskLevel === "DEFINITELY_UNUSED"
        ).length,
