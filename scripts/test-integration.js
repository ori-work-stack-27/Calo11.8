...
class IntegrationTester {
  ...
  async runAllTests() {
    ...
    await this.test("User Registration", async () => {
      const response = await this.makeRequest("POST", "/api/auth/signup", this.testUser);
    ...
    if (this.authToken) {
      ...
      await this.test("Questionnaire API", async () => {
        ...
        const response = await this.makeRequest("POST", "/api/questionnaire", questionnaireData);
        if (!response.data.success) {
      ...
      await this.test("Meal Analysis API", async () => {
        ...
        try {
          const response = await this.makeRequest("POST", "/api/nutrition/analyze", {
          ...
          if (error.response?.data?.error?.includes("OpenAI")) {
            console.log("   ⚠️ AI analysis not available (no API key) - using fallback");
          } else {
            throw error;
          }
        }
      ...
      await this.test("Statistics API", async () => {
        const response = await this.makeRequest("GET", "/api/statistics?period=week");
        if (!response.data.success) {
      ...
      await this.test("Water Intake API", async () => {
        const response = await this.makeRequest("POST", "/api/nutrition/water-intake", {
          cups_consumed: 5,
    ...
    await this.test("Rate Limiting", async () => {
      const requests = [];
      for (let i = 0; i < 20; i++) {
        requests.push(this.makeRequest("GET", "/test").catch(e => e.response));
      }
    ...
    await this.test("CORS Configuration", async () => {
      try {
        const response = await axios.options(`${this.serverUrl}/api/auth/signup`);
  ...
  printResults() {
    ...
    console.log("\n💡 RECOMMENDATIONS:");
    if (this.results.failed === 0) {
      console.log("   🎉 All integration tests passed! Your full stack is working correctly.");
    } else {
      console.log("   🔧 Fix the failed tests above to ensure proper integration.");
