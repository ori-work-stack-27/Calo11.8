...
class CodebaseValidator {
  ...
  validateFileStructure() {
    ...
    // Key files
    const keyFiles = [
      { path: path.join(this.serverPath, "src/index.ts"), name: "Server entry point" },
      { path: path.join(this.serverPath, "prisma/schema.prisma"), name: "Prisma schema" },
      { path: path.join(this.clientPath, "app/_layout.tsx"), name: "Client root layout" },
      { path: path.join(this.clientPath, "app.json"), name: "Expo configuration" },
      { path: path.join(this.clientPath, "src/store/index.ts"), name: "Redux store" },
    ];

    keyFiles.forEach(({ path: filePath, name }) => {
      if (!fs.existsSync(filePath)) {
        this.addIssue(`Missing key file: ${name} (${path.relative(process.cwd(), filePath)})`);
      }
    });
  }
  ...
  validateImports() {
    ...
    allFiles.forEach(filePath => {
      try {
        ...
        imports.forEach(importLine => {
          ...
          if (match) {
            ...
            if (importPath.startsWith("./") || importPath.startsWith("../")) {
              ...
              if (!found) {
                importIssues.add(`${path.relative(process.cwd(), filePath)}: ${importPath}`);
              }
            }
          }
        });
        ...
        importNames.forEach(importBlock => {
          const names = importBlock.match(/import\s+\{([^}]+)\}/)[1]
            .split(",")
            .map(name => name.trim());
          ...
          names.forEach(name => {
            ...
            if (cleanName && !content.includes(cleanName.split(" ")[0])) {
              unusedImports.add(`${path.relative(process.cwd(), filePath)}: ${cleanName}`);
            }
          });
        });
    ...
    if (unusedImports.size > 0 && unusedImports.size < 20) {
      unusedImports.forEach(issue => this.addWarning(`Potentially unused import: ${issue}`));
    }
  }
  ...
  validateConfiguration() {
    ...
    const serverConfigs = [
      { path: path.join(this.serverPath, "package.json"), name: "Server package.json" },
      { path: path.join(this.serverPath, "tsconfig.json"), name: "Server TypeScript config" },
      { path: path.join(this.serverPath, ".env.example"), name: "Server environment template" },
    ];
    ...
    const clientConfigs = [
      { path: path.join(this.clientPath, "package.json"), name: "Client package.json" },
      { path: path.join(this.clientPath, "tsconfig.json"), name: "Client TypeScript config" },
      { path: path.join(this.clientPath, "app.json"), name: "Expo configuration" },
    ];
  ...
  validateCodeQuality() {
    ...
    allFiles.forEach(filePath => {
      try {
        ...
        if (lines > 500) {
          largeFiles.push({ file: path.relative(process.cwd(), filePath), lines });
        }

        // Check for complex functions
        const functionMatches = content.match(/function\s+\w+|const\s+\w+\s*=\s*\(/g) || [];
        if (functionMatches.length > 20) {
          complexFiles.push({ file: path.relative(process.cwd(), filePath), functions: functionMatches.length });
        }
        ...
        if (todos.length > 5) {
          this.addWarning(`Many TODO/FIXME comments in ${path.relative(process.cwd(), filePath)}: ${todos.length}`);
        }
    ...
    if (largeFiles.length > 0) {
      this.addSuggestion(`Consider refactoring large files (>500 lines): ${largeFiles.length} found`);
    ...
    if (complexFiles.length > 0) {
      this.addSuggestion(`Consider breaking down complex files (>20 functions): ${complexFiles.length} found`);
    }

    if (filesWithIssues > 0) {
      this.addWarning(`${filesWithIssues} files contain console.log statements`);
    }
  }
      ...
      if (fs.existsSync(envPath)) {
        ...
        placeholders.forEach(placeholder => {
          if (content.toLowerCase().includes(placeholder)) {
            this.addWarning(`Placeholder value found in ${path.relative(process.cwd(), envPath)}`);
          }
        });
    ...
    allFiles.forEach(filePath => {
      try {
        ...
        suspiciousPatterns.forEach(pattern => {
          ...
          if (matches) {
            this.addWarning(`Potential hardcoded credential in ${path.relative(process.cwd(), filePath)}`);
          }
        });
      } catch (error) {
        // Skip files that can't be read
  ...
  printResults() {
    ...
    if (this.issues.length > 0) {
      ...
      console.log("   2. Consider implementing suggestions for better code quality");
